...
// ReSharper disable InconsistentNaming Ported from c++ and we want to keep naming
...
namespace GrisuDotNet
{
    internal sealed class Bignum
    {
        ...
        public Bignum()
        {
            bigits_ = new uint[kBigitCapacity];
            used_digits_ = 0;
            exponent_ = 0;
        }
        ...
        private uint BigitAt(int index)
        {
        ...
        private void EnsureCapacity(int size)
        {
            ...
            {
                throw new InvalidOperationException($"Size '{size}' cannot be larger than the capacity '{kBigitCapacity}'");
            }
        }
        ...
        private void MultiplyByUInt32(uint factor)
        {
            if (factor == 1)
                return;
            if (factor == 0)
            ...
            while (carry != 0)
            {
                ...
                // NOTE: The following line was not casted in the c++ source,
                // but we're not allowed to disregard the casting here
        ...
        private void SubtractTimes(Bignum other, int factor)
        {
            ...
            {
                ulong product = (ulong) (factor)*other.bigits_[i];
                ulong remove = borrow + product;
                // Note: Introduced specific cast not found in c++ source
                uint difference = (uint) (bigits_[i + exponent_diff] - (remove & kBigitMask));
                bigits_[i + exponent_diff] = difference & kBigitMask;
                borrow = (uint) ((difference >> (kChunkSize - 1)) +
                                 (remove >> kBigitSize));
            }
        ...
        public void AssignPowerUInt16(ushort @base, int power_exponent)
        {
            Debug.Assert(@base != 0);
            ...
            mask >>= 2;
            ulong this_value = @base;

            bool delayed_multipliciation = false;
            ulong max_32bits = 0xFFFFFFFF;
            while (mask != 0 && this_value <= max_32bits)
            {
                ...
                {
                    ulong base_bits_mask = ~((1UL << (64 - bit_size)) - 1);
        ...
        {
            Debug.Assert(kBigitSize >= 8*sizeof(ushort));
            Zero();
            if (value == 0)
                return;
        ...
        public ushort DivideModuloIntBignum(Bignum other)
        {
            ...
            while (BigitLength() > other.BigitLength())
            {
                ...
                Debug.Assert(bigits_[used_digits_ - 1] < 0x10000);
                // Remove the multiples of the first digit.
                // Example this = 23 and other equals 9. -> Remove 2 multiples.
                result += (ushort) (bigits_[used_digits_ - 1]);
                // Note: Introduced specific cast not found in c++ source
                SubtractTimes(other, (int)bigits_[used_digits_ - 1]);
            }
            ...
            uint this_bigit = bigits_[used_digits_ - 1];
            uint other_bigit = other.bigits_[other.used_digits_ - 1];
            ...
            if (other.used_digits_ == 1)
            {
                ...
                // Note: Introduced specific cast not found in c++ source
                bigits_[used_digits_ - 1] = (uint) (this_bigit - other_bigit*quotient);
                Debug.Assert(quotient < 0x10000);
            ...
            Debug.Assert(division_estimate < 0x10000);
            result += (ushort) (division_estimate);
        ...
        public static bool Less(Bignum a, Bignum b)
        {
            return Compare(a, b) < 0;
        }


        public static bool LessEqual(Bignum a, Bignum b)
        {
            return Compare(a, b) <= 0;
        }
